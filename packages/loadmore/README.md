# Overview
sfc-loadmore is a two-direction mobile pull-to-refresh component for vue.js.

# Installation
```bash
$ npm install sfc-loadmore
```

# Usage
Import `sfc-loadmore` to your project:
```Javascript
require('sfc-loadmore/lib/index.css');

// ES6 mudule
import Loadmore from 'sfc-loadmore';

// CommonJS
const Loadmore = require('sfc-loadmore').default;
```
...
<loadmore :top-method="loadTop2" :top-status.sync="topStatus">
  ...
  <div slot="top" class="sfc-loadmore-top">
...
```
At the end of your `top-method`, don't forget to broadcast the `onTopLoaded` event so that `sfc-loadmore` removes `topLoadingText`. Note that a parameter called `id` is passed to `loadTop` and `onTopLoaded`. This is because after the top data is loaded, some reposition work is performed inside a `sfc-loadmore` instance, and `id` simply tells the component which instance should be repositioned. You don't need to do anything more than passing `id` to `onTopLoaded` just as shown above.
