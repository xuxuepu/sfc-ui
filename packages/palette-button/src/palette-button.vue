<template>
  <div class="sfc-palette-button" :class="{ expand: expanded, 'sfc-palette-button-active': transforming }"
    @animationend="onMainAnimationEnd" @webkitAnimationEnd="onMainAnimationEnd" @mozAnimationEnd="onMainAnimationEnd">
    <div class="sfc-sub-button-container">
      <slot></slot>
    </div>
    <div @touchstart="toggle" class="sfc-main-button" :style="mainButtonStyle">
      {{content}}
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    props: {
      ...
      mainButtonStyle: {
        type: String,           // 应用到 sfc-main-button 上的 class
        default: ''
      }
    },
    methods: {
...
<style>
  ...
  .sfc-palette-button-active{
    animation: sfc-zoom 0.5s ease-in-out;
  }
  .sfc-sub-button-container>*{
    position:absolute;
    top:15px;
    left:15px;
    width:25px;
